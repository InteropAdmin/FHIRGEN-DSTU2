...
using Factory = Hl7.Fhir.Publication.Specification.Profile.Example.Index.Factory;

namespace Hl7.Fhir.Publication.Processors.Index
{
    internal class ExampleProcessor : IProcessor
    {
        private const string _fileName = "examples.html";
        ...
        private string Content
        {
            get
            {
                ...
                foreach (Model.ImplementationGuide.PackageComponent package in _baseResource.ImplementationGuide.Package)
                {
                    ...
                    builder.Append(factory.Generate(_inputContext.Root.Target.Directory, _package, _baseResource));
                }
        ...
        {
            _log = log;
            _log.Debug("Examples Index: Start");
            ...
            _log.Debug("Examples Index: Finish");
        }

        private void CreateDirectory()
        {
            string sourceDir = _directoryCreator.EnumerateDirectories(_inputContext.Source.Directory, "Chapter.*.Examples").SingleOrDefault();

            if (string.IsNullOrEmpty(sourceDir))
                throw new InvalidOperationException(" Examples Chapter folder is not found in directory!");
            else
            ...
            _log.Info($"Target examples directory {_targetDir} created");
        }
